  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_cliente($fechadesde,$fechahasta, $condicion) {
            $query = "SELECT*FROM clientes  
            where fecha_apertura >= '$fechadesde' and fecha_apertura <= '$fechahasta' $condicion";  
        ...
        if($cliente_activo!='*' && $clientecontratos=='*'){
            $condicion="AND estado_cliente='".$cliente_activo."'";
        }
        else if($cliente_activo=='*' && $clientecontratos!='*'){
            $condicion="AND posee_contrato='".$clientecontratos."'";

            if($clientecontratos=="Si"){
                $textoconcontrato="CON CONTRATO";
            }
            else{
                $textoconcontrato="SIN CONTRATO";
            }
        }
        else if($cliente_activo!='*' && $clientecontratos!='*'){
            $condicion="AND posee_contrato='".$clientecontratos."' AND estado_cliente='".$cliente_activo."'";
        }
                ...
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    ...
                    <tbody> 
                        <?php
                            ...
                            $data_empleado=fun_cliente($fecha_desde,$fecha_hasta, $condicion);
                            foreach ($data_empleado as $val_emple) {
                              
                            $fecha_apertura = strtotime($val_emple["fecha_apertura"]);
                            $fecha_apertura = date("d-m-Y", $fecha_apertura);
                            $html .= "<tr>";
                            $html .= "<td>".$val_emple["nombre"]."</td>";
                            $html .= "<td>".$fecha_apertura."</td>";
                            $html .= "<td>".$val_emple["estado_cliente"]."</td>";
                            $html .= "<td>".$val_emple["direccion"]."</td>";
                            $html .= "</tr>";
                            }
...
<script>
    ...
    $("#exportPDF").click(function () {
       ...
       fontSize: 5,
        ...
        doc.save('INFORME CONTRATOS PNC.pdf');
                /* ************* */
    })
    ...
    $("#exportExcel").click(function () {
       ...
       /*  ws['!cols'][0] = { wch: 10, };
        ...
        XLSX.writeFile(wb, "INFORME CONTRATOS PNC.xlsx");
