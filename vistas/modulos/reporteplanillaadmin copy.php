    ...
    <div class="box">
     ...
     <a href='todosreportes' class="btn btn-danger">
      ...
      <div class="btnreporte" style="display:none">
       ...
       <a href="ajax/Reporte planilla devengos.txt" download="Reporte planilla devengos.txt" id="descargar_txt" ></a>
      <a href="ajax/Reporte planilla devengos.pdf" download="Reporte planilla devengos.pdf" id="descargar_pdf" ></a>
      <!--  -->
                ...
                <table  class="table table-bordered table-striped dt-responsive " width="100%" id="tabladatos">
                    ...
                    ?>                
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $valorsueldo=floatval($row_data_planilla_admin["sueldo_planilladevengo".$cambio_table.""]);
                                    ...
                                    <td>
                                        ...
                                        /* if($row_data_planilla_admin["descuento_isss_planilladevengo".$cambio_table.""]=="0"){ */
                                    ...
                                    <td>
                                        <?php 
                                            ...
                                            $total_devengados=floatval($valorsueldo)+floatval($vaca)+floatval($capturar_desvengos);
                                            echo bcdiv($total_devengados,'1', 2);
                                            $totaldevengo+=$total_devengados;
                                   ...
                                   <!-- descuento isss -->
                                   <td><?php 
                                   $capturar_isss=floatval($row_data_planilla_admin["descuento_isss_planilladevengo".$cambio_table.""]);
                                   $isss+=floatval($row_data_planilla_admin["descuento_isss_planilladevengo".$cambio_table.""]);
                                    echo bcdiv($row_data_planilla_admin["descuento_isss_planilladevengo".$cambio_table.""],'1', 2)?></td>
                                    <!-- descuento afp -->
                                    <td><?php 
                                   $capturar_afp=floatval($row_data_planilla_admin["descuento_afp_planilladevengo".$cambio_table.""]);
                                     $afp+=floatval($row_data_planilla_admin["descuento_afp_planilladevengo".$cambio_table.""]);
                                    echo bcdiv($row_data_planilla_admin["descuento_afp_planilladevengo".$cambio_table.""],'1', 2)?></td>
                                    <!-- descuento renta -->
                                    <td>
                                        <?php 
                                            $capturar_renta=floatval($row_data_planilla_admin["descuento_renta_planilladevengo".$cambio_table.""]);
                                            $renta+=floatval($row_data_planilla_admin["descuento_renta_planilladevengo".$cambio_table.""]);
                                    ...
                                    <td>
                                        <?php 
                                            $capturar_otrodescuento=floatval($row_data_planilla_admin["otro_descuento_planilladevengo".$cambio_table.""]);
                                            $otrodescuento+=floatval($row_data_planilla_admin["otro_descuento_planilladevengo".$cambio_table.""]);
                                            echo bcdiv($row_data_planilla_admin["otro_descuento_planilladevengo".$cambio_table.""],'1', 2)
                                        ?>
                                    </td>
                                    <td>
                                        <?php
                                             $capturar_totaldescuento=floatval($row_data_planilla_admin["total_descuento_planilladevengo".$cambio_table.""]);
                                             $totaldescuento+=floatval($row_data_planilla_admin["total_descuento_planilladevengo".$cambio_table.""]);
                                             echo bcdiv($row_data_planilla_admin["total_descuento_planilladevengo".$cambio_table.""],'1', 2)
                                        ?>
                                    </td>
                                    <td>
                                        <?php 
                                            $total_devengados=$total_devengados-$capturar_totaldescuento;
                                            echo bcdiv($total_devengados,'1', 2);
                                            $totalliquido+=$total_devengados;
                    ...
                    <tr>
                        ...
                        <!-- otro devengo -->
                        <?php
                            echo "<td>".bcdiv($otrosdevengos,'1', 2)."</td>";   
                        ?>
                        <!-- total devengo -->
                        <?php
                           echo "<td>".bcdiv($totaldevengo,'1', 2)."</td>";    
                        ?>
                        <!-- isss -->
                        <?php
                            $data_sumaglobal = suma_planilla_admin_data($numero,$cambio_table);
                            foreach($data_sumaglobal as $row) {
                                echo "<td>".bcdiv($isss,'1', 2)."</td>";
                            }     
                        ?>
                        <!-- afp -->
                        <?php
                            $data_sumaglobal = suma_planilla_admin_data($numero,$cambio_table);
                            foreach($data_sumaglobal as $row) {
                                echo "<td>".bcdiv($afp,'1', 2)."</td>";
                            }     
                        ?>
                        <!-- renta -->
                         <?php
                          
                                echo "<td>".bcdiv($renta,'1', 2)."</td>";
                               
                        ?>
                        <!-- O. DESC.	 -->
                        <?php
                        
                        echo "<td>".bcdiv($otrodescuento,'1', 2)."</td>";

                        ?>
                        <!-- T. DESC.	 -->
                        <?php
                            
                        echo "<td>".bcdiv($totaldescuento,'1', 2)."</td>";

                        ?>
                        <!-- T. LIQUIDO -->
                         <?php
                                echo "<td>".bcdiv($totalliquido,'1', 2) ."</td>";
...
<script>
  ...
  //Código que se ejecutará al cargar la página
    ...
    $("#exportTXT").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "ajax/pdf_txt_devengos.ajax.php", // Cambia esto por la URL del script PHP en tu servidor
    ...
    $(document).ready(function () {
        ...
        XLSX.writeFile(wb, "Reporte planilla devengos.xlsx");
    });
