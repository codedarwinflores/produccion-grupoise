    ...
    <div class="box">
      ...
      <?php
        ...
        function empleado($fecha) {
            /* $query = "SELECT *
            FROM tbl_empleados
            WHERE STR_TO_DATE(fecha_contratacion, '%d-%m-%Y') < STR_TO_DATE('$fecha', '%d-%m-%Y')"; */
            $query = "SELECT *
            FROM tbl_empleados
        ...
        function empleado_rango($fecha,$empleadodesde, $empleadohasta) {
            /* $query = "SELECT *
            FROM tbl_empleados
            WHERE id>=$empleadodesde and id<=$empleadohasta and  STR_TO_DATE(fecha_contratacion, '%d-%m-%Y') < STR_TO_DATE('$fecha', '%d-%m-%Y')"; */
            $query = "SELECT *
            FROM tbl_empleados
            WHERE id>=$empleadodesde and id<=$empleadohasta and  fecha_contratacion < '$fecha'";
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        ...
        /* $empleado_rango_desde = strtotime($empleado_rango_desde);
        $empleado_rango_desde = date("Y-m-d", $empleado_rango_desde);

        
        $empleado_rango_hasta = strtotime($empleado_rango_hasta);
        $empleado_rango_hasta = date("Y-m-d", $empleado_rango_hasta); */
      ...
      <div class="table-responsive">
                <table  class="table table-bordered table-striped dt-responsive tablas" width="100%" id="tabladatos">
                    <thead>
                        <tr>
                            <th colspan="9">CALCULO DE AGUINALDO</th>
                        </tr>
                        <tr>
                            <th colspan="9"></th>
                        </tr>
                    ...
                    <tbody> 
                        <?php
                            ...
                            if($empleado_rango_desde!="*"){
                                $data_maestra=empleado_rango($fecha_select,$empleado_rango_desde, $empleado_rango_hasta);
                            }
                            else{
                            $data_maestra=empleado($fecha_select);
                            ...
                            foreach ($data_maestra as $val_maestra) { 
                                ...
                                /* 	******CALCULO AGUINALDO******* */
                                    ...
                                    $sueldo_aguinaldo=floatval($val_maestra["sueldo"]);
                                    $suma_sueldo_viatico=$sueldo_aguinaldo+$valor_viaticos;
                                    $salario_mensual=$suma_sueldo_viatico*2;
                                    $salario_diario=$salario_mensual/30;

                                    $valor_aguinaldo=0;
                                    if($sumardiferenciaDias < 365){
                                        /* AGUILNALDO PROPORCIONAL */
                                        $porcentaje=$sumardiferenciaDias/365;
                                        $valor_aguinaldo=$porcentaje*$suma_sueldo_viatico;
                                    }
                                    if($sumardiferenciaDias>365 && $sumardiferenciaDias<1095){
                                    ...
                                    if($sumardiferenciaDias>1095 && $sumardiferenciaDias<3650){
                                        /* AGUINALDO 19 Días de Salario */
                                        $valor_aguinaldo=$salario_diario*19;
                                    }
                                    if($sumardiferenciaDias>3650){
