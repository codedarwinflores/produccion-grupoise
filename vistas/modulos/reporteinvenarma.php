  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function armas($operador) {
            $query = "SELECT * FROM tbl_armas $operador";
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function tipo_arma($tipoarma) {
            $query = "SELECT * FROM tbl_tipos_de_armas $tipoarma";
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
                ...
                <table  class="table table-bordered table-striped dt-responsive tablas" width="100%" id="tabladatos">
                    ...
                    <tbody> 
                        <?php
                            ...
                            $data_maestra=tipo_arma($armatipo);
                            $correlativo=1;
                            foreach ($data_maestra as $val_maestra) { 

                                $id_tipo="WHERE id_tipo_arma='".$val_maestra["id"]."'";
                                
                                $html.="<tr>";
                                $html.="<td colspan='9'>TIPO DE ARMA O EQUIPO : ".$val_maestra["nombre_tipo"]."</td>";
                                $html.="</tr>";  
                                ...
                                $data_arma=armas($id_tipo);
                                foreach ($data_arma as $val_arma) { 

                                    $equipo=$val_arma["codigo"];                       
                                ...
                                $fechaNueva = date("d-m-Y", strtotime($val_arma["fecha_vencimiento"])); // Cambiar el formato a DD/MM/AAAA

                                    $html.="<tr>";
                                    $html.="<td>".$correlativo++." ".$val_arma["marca"]."</td>";
                                    $html.="<td>".$val_arma["numero_serie"]."</td>";
                                    $html.="<td>".$val_arma["tipo_municion"]."</td>";
                                    $html.="<td>".$val_arma["modelo"]."</td>";
                                    $html.="<td>".$fechaNueva."</td>";
