  ...
  <section class="content">
    ...
    <div class="box">
     ...
     <a href='todosreportes' class="btn btn-danger">
      ...
      <a href="ajax/BANCO AGRICOLA.txt" download="BANCO AGRICOLA.txt" id="descargar_txt" ></a>
      ...
      if($tipoplanilla=="planilladevengo_anticipo"){
        ...
        function devengos_norestrinccion($idempleado1,$numero,$devengos_table_maestra) {
            $query = "SELECT SUM(valor_devengo_planilla) as sumadevengo FROM tbl_devengo_descuento_planilla".$devengos_table_maestra." where  idempleado_devengo='$idempleado1' and codigo_planilla_devengo='$numero' and tipo_valor like'%suma%'";
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
                ...
                <table  class="table table-bordered table-striped dt-responsive tablas" width="100%" id="tabladatos">
                    ...
                    ?>                
                                ...
                                <tr>
                                        ...
                                        <td>
                                             <?php 
                                                $devengos=devengos($idempleado,$numero,$devengos_table_maestra);
                                                 foreach($devengos as $row_devengos) {
                                                 $total_global+=bcdiv($row_devengos["sumadevengo"],'1', 2);
                                                 echo  trim(bcdiv($row_devengos["sumadevengo"],'1', 2)).";";
                                                 }
                                             ?>
                                       </td>
                                        <!-- efectivo -->
                                      <td>
                                            <?php
                                                echo trim(str_pad($correlativo, 4, '0', STR_PAD_LEFT)).";";
                                            ?>
                                      </td>
                                      <td>
                                        <?php
                                        ...
                                        echo "PLANILLA MES DE ".$nombre_mes;
                    ...
                    <tr>
                        <td>0522-033146-9;</td>
                        <td>INVESTIGACIONES Y SEGURIDAD S.A. DE C.V.;</td>
                        <td><?php echo trim(bcdiv($total_global,'1', 2))."; ;";?></td>
                        <td><?php echo trim(str_pad($correlativo+1, 4, '0', STR_PAD_LEFT)).";";?></td>
                        <td>
                                <?php
                                        $nombre_mes = date("F", strtotime($fecha_planilla_hasta));
                                        // Traducir el nombre del mes al español
...
<script>
  ...
  //Código que se ejecutará al cargar la página
    ...
    $("#exportTXT").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "ajax/pdf_txt_depoagricola.php", // Cambia esto por la URL del script PHP en tu servidor
            data: { tableData: data },
