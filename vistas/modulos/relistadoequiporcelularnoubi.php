 ...
 ?>
  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_tra_equipo($condicion) {
            $query = "SELECT*FROM movimientosequipos where $condicion AND id_ubicacion_movimiento is null or id_ubicacion_movimiento=''";  
        ...
        function fun_tipo($codigo)
                  {
                        $query01 = "SELECT *FROM celular 
                        where celular.codigo='$codigo' group by celular.operador_celular";
        ...
        function fun_equipo($codigo,$grupo)
                  {
                        $query01 = "SELECT * FROM celular where codigo='$codigo' and operador_celular='$grupo' ";
                        $sql = Conexion::conectar()->prepare($query01);
                        $sql->execute();
                        return $sql->fetchAll();
                  }
    ...
    /*     $fecha_desde = strtotime($fechadesde);
      ...
      <input type="hidden" class="titulo_reportes" value="REPORTE CELULAR">
                    ...
                    <tbody> 
                        <?php
                            ...
                            foreach ($array_datos as $ubicacion) {
                                ...
                                $data_tipo_arma=fun_tipo($codigo);
                                foreach ($data_tipo_arma as $valtipo) {

                                    $name_tipo=$valtipo["operador_celular"];
                                    if (!isset($agrupado[$jefe][$nombre_coor][$name_ubicacion][$name_tipo])) {
                                        $agrupado[$jefe][$nombre_coor][$name_ubicacion][$name_tipo] = array(
                                            ...
                                            'infotipo' => $valtipo["operador_celular"],
                                            'detalles' => array(),
                                        );
                                    } 
                                }/* fin */
                                ...
                                foreach ($jefeData as $nombre_coor => $coorData) {
                                    ...
                                    foreach ($coorData as $nombre_ubi => $ubiData) {
                                        ...
                                        foreach ($tipoData['detalles'] as $ubicacion_detalle) {
                                                ...
                                                $id_tipo=(empty($tipoData["infotipo"])) ? "" : $tipoData["infotipo"];
                                                $data_arma=fun_equipo($codigo,$id_tipo);
                                                foreach ($data_arma as $val_arma) {
                                                    $html .= "<tr>";
                                                    $html .= "<td>".$val_arma["fecha_asignacion_celular"]."</td>";/* fecha asignacion */
                                                    $html .= "<td>".$ubicacion_detalle["fecha_movimiento"]."</td>";/* fecha movimiento */
                                                    $html .= "<td>1"."</td>";/* cantidad */
                                                    $html .= "<td>".$val_arma["descripcion"]."</td>";/* descripcion */
                                                    $html .= "<td>"."</td>";/* matricula */
                                                    $html .= "<td>"."</td>";/* fecha de vencimiento */
                                                    $html .= "<td>".$val_arma["serie_celular"]."</td>";/* serie */
                                                    $html .= "<td>"."</td>";/* estado */
                                                    $html .= "</tr>";
                                                }/* celular */
