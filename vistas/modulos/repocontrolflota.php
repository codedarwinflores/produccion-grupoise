  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_mantenimiento_grupo($fecha_desde, $fecha_hasta, $condicion) {
            $query = "SELECT * FROM mante_vehiculo
            WHERE fecha BETWEEN '$fecha_desde' AND '$fecha_hasta' $condicion 
            GROUP BY idvehiculo_mante 
            order by idvehiculo_mante desc";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        
        function fun_mantenimiento($fecha_desde, $fecha_hasta, $id_vehiculo) {
            $query = "SELECT * FROM mante_vehiculo
            WHERE fecha BETWEEN '$fecha_desde' AND '$fecha_hasta' and idvehiculo_mante= '$id_vehiculo'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_vehiculo($id_vehiculo) {
            $query = "SELECT * FROM tbl_vehiculos
            WHERE id= '$id_vehiculo'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_taller($id_taller) {
            $query = "SELECT * FROM talleres
            WHERE id= '$id_taller'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_empleado($id_empleado) {
            $query = "SELECT * FROM tbl_empleados
            WHERE id= '$id_empleado'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_repara($id_repara) {
            $query = "SELECT * FROM reparaciones
            WHERE id= '$id_repara' and codigo_reparacion='0010' ";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_repara_aceite($id_repara) {
            $query = "SELECT * FROM reparaciones
            WHERE id= '$id_repara' and codigo_reparacion='0004' ";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function patrulla($id_coord) {
            $query = "SELECT tbl_patrullas.* FROM coordinadorpatrulla,tbl_patrullas
            WHERE coordinadorpatrulla.idcoordinador_patrulla= '$id_coord' and 
                coordinadorpatrulla.idpatrulla_coordinadorpatrulla=tbl_patrullas.id";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        ...
        function ultimodato($id_vehi,$id_mante) {
            $query = "SELECT idvehiculo_mante, kilometraje_mvehi
            FROM mante_vehiculo
            WHERE idvehiculo_mante = $id_vehi
            AND id < $id_mante
            ORDER BY id DESC
            LIMIT 1";/*  */  
            $sql = Conexion::conectar()->prepare($query);
        ...
        $condicion="";
        if($vehiculo != "*"){
            $condicion="and idvehiculo_mante='".$vehiculo."'";
        }




        $fecha_desde_for = strtotime($fecha_desde);
        $fecha_desde_for = date("Y-m-d", $fecha_desde_for);
        $fecha_hasta_for = strtotime($fecha_hasta);
        $fecha_hasta_for = date("Y-m-d", $fecha_hasta_for);
      ...
      <input type="hidden" class="titulo_reportes" value="REPORTE CONTROL DE FLOTA ">
      ...
      <div class="table-responsive">
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    <thead>
                        ...
                        <tr>
                            <th class="colspan">CONTROL DE FLOTA </th>
                        </tr>
                        <tr>
                            <th class="colspan">Periodo :<?php echo $fecha_desde."  ".$fecha_hasta?></th>
                        </tr>
                    ...
                    <tbody> 
                        <?php
                            ...
                            $html .= "<tr>";
                            $html .= "<td>No."."</td>";
                            $html .= "<td>UNIDAD ASIGNADA"."</td>";
                            $html .= "<td>RESPONSABLE"."</td>";
                            $html .= "<td>PLACA"."</td>";
                            $html .= "<td>VALOR"."</td>";
                            $html .= "<td>KM ACTUAL"."</td>";
                            $html .= "<td>KM PARA MANTTO."."</td>";
                            $html .= "<td>KM RESTANTES"."</td>";
                            $html .= "<td>CONSUMO ACEITE"."</td>";
                            $html .= "<td>ESTADO"."</td>";
                            $html .= "<td>DIAGNOSTICO"."</td>";
                            $html .= "<td>ENTRA AL TALLER"."</td>";
                            $html .= "<td>SALIDA DEL TALLER"."</td>";
                            $html .= "<td>DIAS EN TALLER"."</td>";
                            $html .= "</tr>";
                            $numero=1;
                            $data_mante=fun_mantenimiento_grupo($fecha_desde_for, $fecha_hasta_for,$condicion);
                            $valor_global=0;
                            foreach ($data_mante as $val_mante) {

                                $id_vehiculo1=$val_mante["idvehiculo_mante"];
                                

                                $km_mante=0;
                                $km_mante_his=0;
                                $km_restante=0;
                                $data_mante_completo=fun_mantenimiento($fecha_desde_for, $fecha_hasta_for, $id_vehiculo1);
                                foreach ($data_mante_completo as $val_vehi_maestra) {
                                    
                                    $id_repara=$val_vehi_maestra["idreparacion_mvehi"];
                                    
                                    $id_vehiculo=$val_vehi_maestra["idvehiculo_mante"];
                                    $id_mante=$val_vehi_maestra["id"];
                                    $id_empleado=$val_vehi_maestra["idempleado_mvehi"];

                                    $data_vehi=fun_vehiculo($id_vehiculo);
                                    $nombre_vehi="";
                                    $codigo_vehi="";
                                    $placa_vehi="";
                                    $estado_vehi="";
                                    foreach ($data_vehi as $val_vehi) {
                                        $nombre_vehi=$val_vehi["descripcion_vehiculo"];
                                        $codigo_vehi=$val_vehi["codigo_vehiculo"];
                                        $placa_vehi=$val_vehi["placa"];
                                        $estado_vehi=$val_vehi["estado_vehiculo"];
                                    }

                                    $data_repa=fun_repara($id_repara);
                                    $nombre_repara="";
                                    $cuenta_repara=0;
                                    foreach ($data_repa as $val_repara) {
                                    $nombre_repara=$val_repara["nombre_reparacion"];
                                    $cuenta_repara++;
                                    }

                                    $data_emple=fun_empleado($id_empleado);
                                    $nombre_cargo="";
                                    ...
                                    $data_patrulla=patrulla($id_empleado);
                                    $patrulla="";
                                    foreach ($data_patrulla as $val_patru) {
                                    $patrulla=$val_patru["descripcion_patrulla"];
                                    }
                                    

                                    
                                    $fecha_ingreso_mvehi0 = date("d-m-Y", strtotime($val_mante["fecha_ingreso_mvehi"]));
                                    $fecha_salida_mvehi0 = date("d-m-Y", strtotime($val_mante["fecha_salida_mvehi"]));

                                    // Fecha de ingreso del vehículo
                                    $fecha_ingreso_mvehi = new DateTime($val_mante["fecha_ingreso_mvehi"]);
                                    // Fecha de salida del vehículo
                                    $fecha_salida_mvehi = new DateTime($val_mante["fecha_salida_mvehi"]);
                                    // Calcula la diferencia entre las dos fechas
                                    $diferencia = $fecha_ingreso_mvehi->diff($fecha_salida_mvehi);
                                    // Obtiene el número de días de diferencia, asegurándose de que sea al menos 1
                                    $num_dias = max(1, $diferencia->days);


                                    $data_ultimo=ultimodato($id_vehiculo,$id_mante);
                                    $kilo_anterior=0;
                                    foreach ($data_ultimo as $val_patru) {
                                    $kilo_anterior=$val_patru["kilometraje_mvehi"];
                                    }

                                    
                                    $data_repa_aceite=fun_repara_aceite($id_repara);
                                    $cuenta_aceite=0;
                                    foreach ($data_repa_aceite as $val_repara) {
                                    $cuenta_aceite++;
                                    }
                                    ...
                                    $config_km_vehi="";
                                    $config_km_moto="";
                                    foreach ($data_config as $val) {
                                        $config_km_vehi=$val["kilo_man_vehi"];
                                        $config_km_moto=$val["kilo_man_moto"];
                                    }


                                    if($cuenta_aceite>0){
                                        if (strpos($codigo_vehi, "MOTO") !== false) {
                                            /* echo "La palabra 'moto' se encuentra en el texto."; */
                                            $km_mante+=$val_vehi_maestra["kilometraje_mvehi"]+$config_km_moto;
                                            $km_mante_his=$km_mante;
                                        } 
                                        else if (strpos($codigo_vehi, "VEHI") !== false){
                                            $km_mante+=$val_vehi_maestra["kilometraje_mvehi"]+$config_km_vehi;
                                            $km_mante_his=$km_mante;
                                        ...
                                        $km_mante=$km_mante_his-$val_vehi_maestra["kilometraje_mvehi"];
                                    }

                                    $km_restante=$val_vehi_maestra["kilometraje_mvehi"]-$km_mante;
                                    
                                    $html .= "<tr>";
                                    $html .= "<td>".$numero++."</td>";
                                    $html .= "<td>".$patrulla."</td>";
                                    $html .= "<td>".$nombre_cargo."</td>";
                                    $html .= "<td>".$placa_vehi."</td>";
                                    $html .= "<td>".$val_vehi_maestra["valor_mvehi"]."</td>";
                                    $html .= "<td>".$val_vehi_maestra["kilometraje_mvehi"]."</td>";
                                    $html .= "<td>".$km_mante."</td>";/* KM PARA MANTTO */
                                    $html .= "<td>".$km_restante."</td>";/* KM RESTANTES */
                                    $html .= "<td>0.00%"."</td>";/*  */
                                    $html .= "<td>".$estado_vehi."</td>";
                                    $html .= "<td>".$val_vehi_maestra["diagnostico_mvehi"]."</td>";
                                    $html .= "<td>".$fecha_ingreso_mvehi0."</td>";
                                    $html .= "<td>".$fecha_salida_mvehi0."</td>";
                                    $html .= "<td>".$num_dias."</td>";
                                    $html .= "</tr>";
