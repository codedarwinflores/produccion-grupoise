  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function funplanillaadminafpnoemple() {
            $query = "SELECT sum(descuento_isss_planilladevengo_admin) as totalisss,
                             sum(descuento_afp_planilladevengo_admin) as totalafp,
        ...
        function devengos_contodo($idempleado1,$mes) {
            /* $query = "SELECT sum(sueldo_planilladevengo_admin) as sueldo, SUM(descuento_renta_planilladevengo_admin) as renta, sum(descuento_afp_planilladevengo_admin) as afp, SUM(valor_devengo_planilla) AS sumadevengo, idempleado_devengo
            ...
            /* $query="SELECT
            SUM(pda.sueldo_planilladevengo_admin) AS sueldo,
            SUM(pda.descuento_renta_planilladevengo_admin) AS renta,
            SUM(pda.descuento_afp_planilladevengo_admin) AS afp,
            SUM(ddp.valor_devengo_planilla) AS sumadevengo,
            ddp.idempleado_devengo,
            pda.numero_planilladevengo_admin
            FROM
                planilladevengo_admin pda
            LEFT JOIN
                tbl_devengo_descuento_planilla_admin ddp ON pda.numero_planilladevengo_admin = ddp.codigo_planilla_devengo
            WHERE
                MONTH(STR_TO_DATE(pda.fecha_planilladevengo_admin, '%d-%m-%Y')) = '$mes'
                AND ddp.idempleado_devengo = '$idempleado1'
                AND ddp.tipo_valor LIKE '%suma%'
                AND (
                    ddp.isss_devengo_devengo_descuento_planilla = 'Si'
                    OR ddp.afp_devengo_devengo_descuento_planilla = 'Si'
                    OR ddp.renta_devengo_devengo_descuento_planilla = 'Si'
                )
            GROUP BY ddp.idempleado_devengo"; */
            /* $query="SELECT
                    SUM(pda.sueldo_planilladevengo_admin) AS sueldo,
                    SUM(pda.descuento_renta_planilladevengo_admin) AS renta,
                    SUM(pda.descuento_afp_planilladevengo_admin) AS afp,
                    SUM(ddp.valor_devengo_planilla) AS sumadevengo,
                    pda.id_empleado_planilladevengo_admin
                FROM
                    tbl_devengo_descuento_planilla_admin ddp
                LEFT JOIN
                    planilladevengo_admin pda ON ddp.codigo_planilla_devengo = pda.numero_planilladevengo_admin
                    AND MONTH(STR_TO_DATE(pda.fecha_planilladevengo_admin, '%d-%m-%Y')) = '$mes'
                WHERE
                    ddp.idempleado_devengo = '$idempleado1'
                    AND ddp.tipo_valor LIKE '%suma%'
                    AND (
                        ddp.isss_devengo_devengo_descuento_planilla = 'Si'
                        OR ddp.afp_devengo_devengo_descuento_planilla = 'Si'
            ...
            FROM tbl_devengo_descuento_planilla_admin,planilladevengo_admin
            WHERE MONTH(STR_TO_DATE(fecha_planilladevengo_admin, '%d-%m-%Y')) = '$mes' and
            planilladevengo_admin.id_empleado_planilladevengo_admin = '$idempleado1' and  tipo_valor like '%suma%'
                ...
                AND (isss_devengo_devengo_descuento_planilla = 'Si'
                    OR
                    afp_devengo_devengo_descuento_planilla = 'Si'
                    OR
        ...
        function planillaadminmes($idempleado1,$mes) {
            ...
            where id_empleado_planilladevengo_admin='$idempleado1' and 
            MONTH(STR_TO_DATE(fecha_planilladevengo_admin, '%d-%m-%Y')) = '$mes'";
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            $resultados=$sql->fetchAll();	
            $sql=null;
        ...
        function planillaadminemple($idempleado1) {
                ...
                <table  class="table table-bordered table-striped dt-responsive tablas" width="100%" id="tabladatos">
                    ...
                    <tbody> 
                        <?php
                            ...
                            /* array */
                            $data_afp=funplanillaadminafpnoemple();
                            $array_afp=[];
                            ...
                            foreach ($array_depa_maestra as $val_maestra) {
                                ...
                                foreach ($array_empleados_admin as $val_empleado_maestro) {
                                 foreach ($val_empleado_maestro["detalle"] as $val_empleado) {
                                        ...
                                        if ($val_empleado["id_departamento_empresa"] == $id_depa) {
                                            ...
                                            $data_totalemple=planillaadminemple($idempleado_admin);
                                            $capture_devengado=0;
                                            $capture_liquido=0;
                                            $capture_renta=0;
                                            ...
                                            foreach ($data_aguinaldo as $val_aguinaldo) {
                                                # code...
                                                if($val_aguinaldo["total_liquidado_planilladevengo_aguinaldo"]>="608.34"){
                                                    $aguinaldo=608.34;
                                                }
                                                else{
                                            ...
                                            for ($i = 1; $i <= 12; $i++) {
                                                    ...
                                                    $total_afp_enero=0;
                                                    $sueldo_admin=0;
                                                    $valor_devengo_mensual=0;
                                                    $data_plames=planillaadminmes($idempleado_admin,$i);
                                                    foreach ($data_plames as $datos_plames) {
                                                        $sueldo_admin=floatval($datos_plames["sueldo"]);
                                                        $total_renta_enero=floatval($datos_plames["renta"]);
                                                        $total_afp_enero=floatval($datos_plames["afp"]);
                                                    }
                                                    $data_devengos_mes=devengos_contodo($idempleado_admin,$i);
                                                    foreach ($data_devengos_mes as $datos_mes) {
                                                        $valor_devengo_mensual=floatval($datos_mes["sumadevengo"]);
                                                    }
                                                    $total_gravado_enero=$sueldo_admin+$valor_devengo_mensual;
                                                        
                                                    $html.="<td>".bcdiv($total_gravado_enero,'1', 2)."</td>";
                                                    $html.="<td>".bcdiv($total_renta_enero,'1', 2)."</td>";
                                                    $html.="<td>".bcdiv($total_afp_enero,'1', 2)."</td>";
                                            }
