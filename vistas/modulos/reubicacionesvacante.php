                            ...
                            foreach ($data_maestra as $value) {
                              ...
                              foreach ($data_ubi as $subvalue) {
                               ...
                               foreach ($data_asigna as $val_asig) {
                                   ...
                                   if($partes[0]==$subvalue["codigo_ubicacion"]){
                                       $asig++;
                                   }
                               }
                                $data_auto=fun_autorizados($subvalue["codigo_ubicacion"],$fechahasta_formateada);
                                $auto=0;
                                foreach ($data_auto as $val_auto) {
                                    $auto=$val_auto["actual_hombre"];
                                }
                                $vacante=floatval($auto)-floatval($asig);
                                if(floatval($vacante)<0){
                                    $vacante=0;
                                }
