    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_tra_equipo_actual($fecha, $condicion) {
            ...
            ) and STR_TO_DATE(fecha_movimiento_his, '%d-%m-%Y') <= STR_TO_DATE('$fecha', '%d-%m-%Y') and tipoequipo_his='arma' or tipoequipo_his='radio' or tipoequipo_his='celular' $condicion
            ORDER BY h1.tipoequipo_his desc";  
        ...
        function radios($codigo)
                  {
                        $query01 = "SELECT * FROM tbl_radios where codigo_radio='$codigo'";
                        $sql = Conexion::conectar()->prepare($query01);
                        $sql->execute();
                        return $sql->fetchAll();
                  }
        ...
        function celular($codigo)
        {
              $query01 = "SELECT * FROM celular where codigo='$codigo'";
              $sql = Conexion::conectar()->prepare($query01);
              $sql->execute();
              return $sql->fetchAll();
        }
                        ...
                        <?php
                            ...
                            foreach ($data_movi_his as $va_movi) {
                                ...
                                foreach ($data_arma as $val_arma) {
                                    $nom_equipo=$val_arma["descripcion_arma"];
                                    $matri_equipo=$val_arma["numero_matricula"];
                                    $venci_equipo=$val_arma["fecha_vencimiento"];
                                    $serie_equipo=$val_arma["numero_serie"];
                                }

                                /* radio**************** */
                                $data_arma=radios($codigo_equipo);
                                foreach ($data_arma as $val_arma) {
                                    $nom_equipo=$val_arma["descripcion_radio"];
                                    $matri_equipo=""/* $val_arma["numero_matricula"] */;
                                    $venci_equipo=""/* $val_arma["fecha_vencimiento"] */;
                                    $serie_equipo=$val_arma["numero_serie"];
                                }
                                ...
                                $data_arma=celular($codigo_equipo);
                                foreach ($data_arma as $val_arma) {
                                    $nom_equipo=$val_arma["descripcion"];
                                    $matri_equipo=""/* $val_arma["numero_matricula"] */;
                                    $venci_equipo=""/* $val_arma["fecha_vencimiento"] */;
                                    $serie_equipo=$val_arma["serie_celular"];
                                }
                                ...
                                $venci_equipo = strtotime($venci_equipo);
                                $venci_equipo = date("d-m-Y", $venci_equipo);

                                $html .= "<tr>";
                                $html .= "<td>".$cadena_con_ceros."</td>";
                                $html .= "<td>".$va_movi["fecha_ingreso_movimiento_his"]."</td>";
                                $html .= "<td>".$va_movi["fecha_movimiento_his"]."</td>";
                                $html .= "<td>".$nom_equipo."</td>";
                                $html .= "<td>".$matri_equipo."</td>";
                                $html .= "<td>".$venci_equipo."</td>";
                                $html .= "<td>".$serie_equipo."</td>";
