  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_situacion($fecha_desde,$fecha_hasta,$condicion) {
            $query = "SELECT * FROM situacion
            WHERE STR_TO_DATE(fecha_situacion, '%d-%m-%Y') >= STR_TO_DATE('$fecha_desde', '%d-%m-%Y') and 
                  STR_TO_DATE(fecha_situacion, '%d-%m-%Y') <= STR_TO_DATE('$fecha_hasta', '%d-%m-%Y') AND 
                  horas_no_cubiertas != '' and horas_no_cubiertas != '0' $condicion";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_situacion_ubi($fecha_desde,$fecha_hasta,$codigo_ubi) {
            $query = "SELECT * FROM situacion
            WHERE STR_TO_DATE(fecha_situacion, '%d-%m-%Y') >= STR_TO_DATE('$fecha_desde', '%d-%m-%Y') and 
                  STR_TO_DATE(fecha_situacion, '%d-%m-%Y') <= STR_TO_DATE('$fecha_hasta', '%d-%m-%Y') AND 
                  horas_no_cubiertas != '' and horas_no_cubiertas != '0' and ubicacion_situacion like '%$codigo_ubi%' ";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_cliente($id_cliente) {
            $query = "SELECT * FROM clientes
            WHERE  id = $id_cliente ";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        ...
        function fun_empleado($codigo_emple) {
            $query = "SELECT * FROM tbl_empleados where codigo_empleado='$codigo_emple'";
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_motivo($id_motivo) {
        ...
        function fun_factura($id_ubi) {
            $query = "SELECT * FROM tbl_ubicaciones_detalle where  idubicacion = $id_ubi";
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_ubicacion($codigo) {
            $query = "SELECT * FROM tbl_clientes_ubicaciones where  codigo_ubicacion = '$codigo'";
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        ...
        $condicion="";
        if($ubicacion != "*"){
            $condicion="and ubicacion_situacion like'%".$ubicacion."%'";
        }
      ...
      <input type="hidden" class="titulo_reportes" value="CALCULO DE AGUINALDOS">
        
      <div class="table-responsive">
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    <thead>
                        <tr>
                            <th class="colspan">CALCULO DE AGUINALDOS</th>
                        </tr>
                        <tr>
                            <th class="colspan"><?php echo $fecha_desde?></th>
                        </tr>
                        <tr>
                            <th class="colspan"></th>
                        </tr>
                    ...
                    <tbody> 
                        <?php
                            ...
                            if($cliente_input=="*"){
                                $data_situ=fun_situacion($fecha_desde,$fecha_hasta,$condicion);
                                foreach ($data_situ as $val_situ) {
                                $cuenta_registros++;
                                $codigo_emple=$val_situ["idempleado_situacion"];
                                $motivo=explode("-", $val_situ["motivo_horas_extras"]);
                                $codigo_ubicacion=explode("-", $val_situ["ubicacion_situacion"]);
                                $codigo_emple_cubrio=explode("-", $val_situ["cubrir_situacion"]);


                                $data_emple=fun_empleado($codigo_emple);
                                $nombres="";
                                $apellidos="";
                                foreach ($data_emple as $val_emple) {
                                    $nombres=trim(trim($val_emple["primer_nombre"])." ".trim($val_emple["segundo_nombre"]).' '.trim($val_emple["tercer_nombre"]));
                                    $nombres = preg_replace('/\s+/', ' ', $nombres);

                                    $apellidos=trim(trim($val_emple["primer_apellido"])." ".trim($val_emple["segundo_apellido"]).' '.trim($val_emple["apellido_casada"]));
                                    $apellidos = preg_replace('/\s+/', ' ', $apellidos);
                                }

                                /* empleado que cubrio  */

                                $data_emplecubrir=fun_empleado($codigo_emple_cubrio[0]);
                                $nombres_cubri="";
                                $apellidos_cubri="";
                                foreach ($data_emplecubrir as $val_emple) {
                                    $nombres_cubri=trim(trim($val_emple["primer_nombre"])." ".trim($val_emple["segundo_nombre"]).' '.trim($val_emple["tercer_nombre"]));
                                    $nombres_cubri = preg_replace('/\s+/', ' ', $nombres_cubri);

                                    $apellidos_cubri=trim(trim($val_emple["primer_apellido"])." ".trim($val_emple["segundo_apellido"]).' '.trim($val_emple["apellido_casada"]));
                                    $apellidos_cubri = preg_replace('/\s+/', ' ', $apellidos_cubri);
                                }
                                /* ************ */

                                $data_motivo=fun_motivo($motivo[0]);
                                $nombre_motivo="";
                                foreach ($data_motivo as $val_motivo) {
                                    $nombre_motivo=$val_motivo["motivo_tipohora"];
                                }

                                $data_ubi=fun_ubicacion($codigo_ubicacion[0]);
                                $nombre_ubi="";
                                $valor_factura="";
                                foreach ($data_ubi as $val_ubi) {
                                    ...
                                    $data_factu=fun_factura($id_ubi);
                                    foreach ($data_factu as $val_fac) {
                                    $valor_factura_entre30=floatval($val_fac["valor"])/30;
                                    $valor_factura=$valor_factura_entre30/12;
                                    }
                                }

                                $cubiertas=12-$val_situ["horas_no_cubiertas"];

                                $html .= "<tr>";
                                $html .= "<td>".$cuenta_registros."</td>";
                                $html .= "<td>".$val_situ["fecha_situacion"]."</td>";
                                $html .= "<td>".$codigo_emple."</td>";
                                $html .= "<td>".$nombres."</td>";
                                $html .= "<td>".$apellidos."</td>";
                                $html .= "<td>".$nombre_motivo."</td>";
                                $html .= "<td>".$val_situ["horas_no_cubiertas"]."</td>";
                                $html .= "<td>".bcdiv($valor_factura,'1', 2)."</td>";
                                $html .= "<td>".$cubiertas."</td>";
                                $html .= "<td>".$nombre_ubi."</td>";
                                $html .= "<td>".$apellidos_cubri." ".$nombres_cubri."</td>";
                                $html .= "</tr>";
                                }
                            }
                            else{

                                $data_cliente=fun_cliente($cliente_input);
                                foreach ($data_cliente as $all_cliente) {
                                    $codigo=$all_cliente["codigo"];
                                        /* ******************** */
                                            $data_situ=fun_situacion_ubi($fecha_desde,$fecha_hasta,$codigo);
                                            foreach ($data_situ as $val_situ) {
                                            $cuenta_registros++;
                                            $codigo_emple=$val_situ["idempleado_situacion"];
                                            $motivo=explode("-", $val_situ["motivo_horas_extras"]);
