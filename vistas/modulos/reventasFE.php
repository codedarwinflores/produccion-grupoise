  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_facturar($tipo,$tiempo) {
            $query = "SELECT tbl_ubicaciones_detalle.*, SUM(total) as totalsuma, COUNT(*) as cantidad_registros FROM `tbl_ubicaciones_detalle` where tipo_documento='$tipo' and forma_pago='$tiempo'
            group by facturar order by id desc";/* 2023-12-31 formato fecha */             
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        function fun_facturar_detalle($tipo,$tiempo,$facturar) {
            $query = "SELECT * FROM `tbl_ubicaciones_detalle` where tipo_documento='$tipo' and forma_pago='$tiempo' and facturar='$facturar'
            order by id desc";/* 2023-12-31 formato fecha */             
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_cliente($nombre) {
            $query = "SELECT * FROM `clientes` where nombre='$nombre'";/* 2023-12-31 formato fecha */  
        ...
        function fun_series($tipo) {
            $query = "SELECT * FROM `series_ventas` where tipo_serie='$tipo'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        ...
        function fun_config() {
            $query = "SELECT * FROM `configuracion`";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        ...
        $data_series=fun_series($tipo);
        $series_tipo="";
        $cuenta_contable="";
        foreach ($data_series as $value) {
            $series_tipo=$value["num_serie"];
            $cuenta_contable=$value["cuenta_contable"];
        }

        $fata_config=fun_config();
        $cuenta_cobrar="";
        $cuenta_venta="";
        foreach ($fata_config as $value) {
            $cuenta_cobrar=$value["cxc"];
            $cuenta_venta=$value["cuentas_ventas"];
      ...
      <input type="hidden" class="titulo_reportes" value="VENTAS-CCF">
                ...
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    ...
                    <tbody> 
                        <?php
                            ...
                            $data_factura=fun_facturar($tipo,$tiempo);
                            foreach ($data_factura as $val_maestra) {
                                $nombre_cliente=trim($val_maestra["facturar"]);

                                $data_cliente=fun_cliente($nombre_cliente);
                                $codigo_cliente="";
                                $dias_plazo=0;
                                $categoria_cliente="EXEFIN";
                                foreach ($data_cliente as $val_cliente) {
                                    $codigo_cliente=$val_cliente["codigo"];
                                    $dias_plazo=$val_cliente["plazo"];
                                    /* if($val_cliente["categoria_cliente"]!="Grande"){
                                        $categoria_cliente="IVACON";
                                    }
                                    else{
                                        $categoria_cliente="IVAGRAN";
                                    } */
                                }
                                                                
                                // Fecha inicial
                                $fechaInicial = DateTime::createFromFormat('d-m-Y', $fecha_hasta);
                                // Sumar 30 días
                                $fechaFinal = $fechaInicial->modify('+'.$dias_plazo.' days');
                                // Imprimir la fecha resultante en formato dd-mm-yyyy
                                $fechasumada= $fechaFinal->format('d-m-Y');

                                $calculo_iva=$val_maestra["totalsuma"]*0.13;

                                
                                
                                $cantidad_registros=$val_maestra["cantidad_registros"]+1;

                                $html.="<tr style='display:none'>";
                                $html.="<td>".$codigo_cliente."</td>";
                                $html.="<td>".$tipo.$codigo."-".$series_tipo."</td>";
                                $html.="<td>".$fecha_hasta."</td>";
                                $html.="<td>".$fechasumada."</td>";
                                $html.="<td>".$cuenta_contable."</td>";
                                $html.="<td>".$categoria_cliente."</td>";
                                $html.="<td>IVA CONSUMIDOR FINAL"."</td>";
                                $html.="<td>0"."</td>";
                                $html.="<td>0"."</td>";
                                $html.="<td>"."-".bcdiv($calculo_iva, '1', 2)."</td>";
                                $html.="<td>".$cantidad_registros."</td>";
                                $html.="<td>FALSO"."</td>";
                                $html.="<td>".$cuenta_cobrar."</td>";
                                $html.="<td>0"."</td>";
                                $html.="<td>".$categoria_cliente."</td>";
                                $html.="<td>"."</td>";
                                $html.="</tr>";

                                $data_detalle_factura=fun_facturar_detalle($tipo,$tiempo,$nombre_cliente);
                                foreach ($data_detalle_factura as $val_detalle) {


                                    /* *************** */
                                        // Fecha en formato dd-mm-yyyy
                                        $fechaString = $fecha_hasta;
                                        ...
                                        $numeroMes = $fecha->format('n');  // 'n' devuelve el número del mes (sin ceros iniciales)
                                        $nombreMes = $nombresMeses[$numeroMes];
                                        $ano = $fecha->format('Y');        // 'Y' devuelve el año en formato de 4 dígitos
                                    /* ************** */

                                    $suma_iva_total=$val_maestra["totalsuma"];
                                    ...
                                    $html.="<td>".$fecha_hasta."</td>";
                                    $html.="<td>".$fechasumada."</td>";
                                    $html.="<td>".$cuenta_venta."</td>";/* pendiente */
                                    $html.="<td>".$categoria_cliente."</td>";
                                    $html.="<td>".$val_detalle["concepto"]." ".$nombreMes." / ".$ano."</td>";
                                    $html.="<td>".$val_detalle["numero_hombres"]."</td>";
                                    $html.="<td>".$val_detalle["precio"]."</td>";
                                    $html.="<td>"."-".$val_detalle["total"]."</td>";
                                    $html.="<td>".$cantidad_registros."</td>";
                                    $html.="<td>FALSO"."</td>";
                                    $html.="<td>".$cuenta_cobrar."</td>";
                                    $html.="<td>1"."</td>";
                                    $html.="<td>"."</td>";
                                    $html.="<td>".numeroALetrasConDecimales(bcdiv($suma_iva_total, '1', 2))."</td>";
                                    $html.="</tr>";
                                }
                                $codigo=$codigo+1;
                            ...
                            function numeroALetras($numero) {
                                ...
                                if ($numero < 10) {
                                    ...
                                    $unidadEspecial = array('Diez', 'Once', 'Doce', 'Trece', 'Catorce', 'Quince', 'Dieciséis', 'Diecisiete', 'Dieciocho', 'Diecinueve');
                                    return $unidadEspecial[$numero - 10];
                                } elseif ($numero >= 20 && $numero < 100) {
                                    $decena = (int)($numero / 10);
                                    $resto = $numero % 10;
                                    return $decenas[$decena] . ($resto > 0 ? ' y ' . $unidad[$resto] : '');
                                } elseif ($numero >= 100 && $numero < 1000) {
                                    $centena = (int)($numero / 100);
                                    $resto = $numero % 100;
                                    $resultado = $centenas[$centena];
                                    if ($resto > 0) {
                                        $resultado .= ($resto < 10 ? ' y ' : ' ') . numeroALetras($resto);
                                    }
                                    return $resultado;
                                    ...
                                    if ($resto > 0) {
                                        $resultado .= ($resto < 100 ? ' ' : ' ') . numeroALetras($resto);
                                    }
                                    return $resultado;
                                } else {
