                ...
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    ...
                    <tbody> 
                        <?php
                            ...
                            $formapago_mostradas = array();
                            $tipodocumento_mostradas = array();
                            $rubro_mostradas = array();

                            foreach ($array_ubicacion as $value) {
                                $forma_pago= $value["forma_pago"];
                                $tipo_documento= $value["tipo_documento"];
                                $rubro= $value["rubro"];


                                if (!in_array($rubro, $rubro_mostradas)) {
                                    $html .= "<tr>";
                                    $html .= "<td class='colspan'>RUBRO " . $rubro . "</td>";
                                    $html .= "</tr>";
                                    $rubro_mostradas[] = $rubro;
                                }

                                if (!in_array($forma_pago, $formapago_mostradas) && $rubro == $rubro_mostradas[count($rubro_mostradas) - 1]) {
                                    $html.="<tr>";
                                    $html.="<td class='colspan'>FORMA DE PAGO : ".$forma_pago."</td>";
                                    $html.="</tr>";
                                    $formapago_mostradas[] = $forma_pago;
                                }


                                if (!in_array($tipo_documento, $tipodocumento_mostradas) && $rubro == $rubro_mostradas[count($rubro_mostradas) - 1] && $forma_pago == $formapago_mostradas[count($formapago_mostradas) - 1]) {
                                    $html .= "<tr>";
                                    $html .= "<td class='colspan'>DOCUMENTO " . $tipo_documento . "</td>";
                                    $html .= "</tr>";
                                    $tipodocumento_mostradas[] = $tipo_documento;
                                }

                                

                                if ($rubro == $rubro_mostradas[count($rubro_mostradas) - 1] || $forma_pago == $formapago_mostradas[count($formapago_mostradas) - 1] || $tipo_documento == $tipodocumento_mostradas[count($tipodocumento_mostradas) - 1]) {

                                    $html .= "<tr>";
                                    $html .= "<td >" . $value["codigo_ubicacion"] . "</td>";
                                    $html .= "<td >" . $value["nombre_ubicacion"] . "</td>";
                                    $html .= "<td >" . $value["aut"] . "</td>";
                                    $html .= "<td >" . $value["facturar"] . "</td>";
                                    $html .= "<td >?" . "</td>";
                                    $html .= "<td >" . $value["precio"] . "</td>";
                                    $html .= "<td >" . $value["total"] . "</td>";
                                    $html .= "<td >" . $value["porcen"] . "</td>";
