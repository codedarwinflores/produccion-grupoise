  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_radios($estado) {
            $query = "SELECT * FROM `tbl_radios` where estado_radio='$estado'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_ubicacion($codigo_equipo) {
            $query = "SELECT tbl_clientes_ubicaciones.* 
            FROM `movimientosequipos`,tbl_clientes_ubicaciones
             where tbl_clientes_ubicaciones.id=movimientosequipos.id_ubicacion_movimiento and 
                   codigo_equipo='$codigo_equipo' 
             group by codigo_equipo order by id desc";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
      ...
      <input type="hidden" class="titulo_reportes" value="REPORTE LISTADO DE ARMAS BOTON RADIOS">
      ...
      <div class="table-responsive">
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    <thead>
                        ...
                        <tr>
                            <th class="colspan">LISTADO DE RADIOS</th>
                        </tr>
                        <tr>
                            <th class="colspan"></th>
                        </tr>
                    ...
                    <tbody> 
                        <?php
                            ...
                            $html.="<tr>";
                            $html.="<td class='colspan'>ACTIVOS"."</td>";
                            $html.="</tr>";

                            $data_radios=fun_radios("Activo");
                            foreach ($data_radios as $val_radio) {
                                $act_desc=$val_radio["estado_radio"];
                                $codigo_equipo=$val_radio["codigo_radio"];

                                $data_ubi=fun_ubicacion($codigo_equipo);
                                $nombre_ubi="";
                                foreach ($data_ubi as $val_ubi) {
                                    ...
                                    $html .= "<tr>";
                                    $html .= "<td>".$val_radio["numero_serie"]."</td>";
                                    $html .= "<td>".$val_radio["descripcion_radio"]."</td>";
                            ...
                            $html.="<tr>";
                            $html.="<td class='colspan'>INACTIVOS"."</td>";
                            $html.="</tr>";

                            $data_radios=fun_radios("Inactivo");
                            foreach ($data_radios as $val_radio) {
                                $act_desc=$val_radio["estado_radio"];
                                $codigo_equipo=$val_radio["codigo_radio"];
