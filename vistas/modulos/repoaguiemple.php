  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_empleado() {
            $query = "SELECT * FROM tbl_empleados
            WHERE estado= '2'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_empleado_ubi($ubi_emple) {
            $query = "SELECT * FROM transacciones_agente WHERE idagente_transacciones_agente = '$ubi_emple' ORDER BY id DESC LIMIT 1";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };

        function fun_ubi($codigo) {
            $query = "SELECT * FROM tbl_clientes_ubicaciones WHERE codigo_ubicacion	 = '$codigo'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
                ...
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    ...
                    <tbody> 
                        <?php
                            ...
                            $data_empleados=fun_empleado();
                            $inde_global=0;
                            $porcentaje_goblal=0;
                            foreach ($data_empleados as $value) {
                               
                                $ubi_emple=$value["codigo_empleado"];

                                $nombres=trim(trim($value["primer_nombre"])." ".trim($value["segundo_nombre"]).' '.trim($value["tercer_nombre"]));
                                $nombres = preg_replace('/\s+/', ' ', $nombres);
                                
                                $sueldo=floatval($value["sueldo"])*2;

                                $fecha_desde_for = strtotime($value["fecha_ingreso"]);
                                $fecha_desde_for = date("d-m-Y", $fecha_desde_for);

                                /* calculo de dias */
                                $fecha_contratacion = strtotime($value["fecha_contratacion"]);
                                $fecha_contratacion = date("Y-m-d", $fecha_contratacion);

                                $fecha_actual = strtotime($fecha_desde);
                                $fecha_actual = date("Y-m-d", $fecha_actual);

                                $fechaInicio = new DateTime($fecha_contratacion);
                                ...
                                /* calculo inde */
                                $inde=bcdiv($sueldo,'1', 2)/365*$numDias;
                                /*  */

                                /* calculo 50% */
                                $porcentaje=bcdiv($inde,'1', 2)*0.50;
                                /*  */

                                /* calculo aguinaldo proporcional */
                                $valor_aguinaldo=0;
                                $salario_diario=$sueldo/30;
                                if($numDias < 365){
                                    /* AGUILNALDO PROPORCIONAL */
                                    $porcentaje=$numDias/365;
                                    $valor_aguinaldo=$porcentaje*$sueldo;
                                }
                                ...
                                $data_ubi_emple=fun_empleado_ubi($ubi_emple);
                                $nombre_ubi="";
                                foreach ($data_ubi_emple as $val_ubi) {
                                    $codigo_ubi= explode("-", $val_ubi["nueva_ubicacion_transacciones_agente"]);
                                    $data_ubica=fun_ubi($codigo_ubi[0]);
                                    foreach ($data_ubica as $val_detalle) {
                                        $nombre_ubi=$val_detalle["nombre_ubicacion"];
                                    }
                                ...
                                $html .= "<tr>";
                                $html .= "<td>".$value["codigo_empleado"]."</td>";
                                $html .= "<td>".$nombres."</td>";
                                $html .= "<td>".$value["primer_apellido"]."</td>";
                                $html .= "<td>".$value["segundo_apellido"]."</td>";
                                $html .= "<td>$ ".bcdiv($sueldo,'1', 2)."</td>";
                                $html .= "<td>".$fecha_desde_for."</td>";
                                $html .= "<td>".$numDias."</td>";
                                $html .= "<td>$ ".bcdiv($valor_aguinaldo,'1', 2)."</td>";
                                $html .= "<td>".$nombre_ubi."</td>";
                                $html .= "</tr>";

                                $inde_global+=bcdiv($valor_aguinaldo,'1', 2);
                                /* $porcentaje_goblal+=bcdiv($valor_aguinaldo,'1', 2); */
                            }
                            $html .= "<tr>";
                            $html .= "<td>"."</td>";
                            $html .= "<td>"."</td>";
                            ...
                            $html .= "<td>"."</td>";
                            $html .= "<td>TOTALES"."</td>";
                            $html .= "<td>$".bcdiv($inde_global,'1', 2)."</td>";
                            $html .= "<td>"."</td>";
