  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_tra_agente($turnos,$condicion) {
            $query = "SELECT*FROM transacciones_agente
            WHERE turno_transacciones_agente='$turnos' $condicion
        ...
        $condicion="";
        if($ubicacion != "*"){
            $condicion="and nueva_ubicacion_transacciones_agente like '%$ubicacion%'";
        }
    ...
    /*     $fecha_desde = strtotime($fechadesde);
      ...
      <input type="hidden" class="titulo_reportes" value="REPORTE TURNOS">
      ...
      <div class="table-responsive">
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    <thead>
                        ...
                        <tr>
                            <th class="colspan">*** LISTADO DE EMPLEADOS POR TURNOS/<?php echo $turnos?> *** </th>
                        </tr>
                        <tr>
                    ...
                    <tbody> 
                        <?php
                            ...
                            $data_tran_agente=fun_tra_agente($turnos,$condicion);
                            foreach ($data_tran_agente as $val_tran) {
                                $codigo_agente=$val_tran["idagente_transacciones_agente"];
                                $codigo_ubicacion=explode("-", $val_tran["nueva_ubicacion_transacciones_agente"]);
                                
                                $data_empleado=fun_empleado($codigo_agente);
                                $nombre_cargo="";
                                ...
                                $nombre_ubica="";
                                $idubi="";
                                foreach ($data_ubi as $value) {
                                    $nombre_ubica=$value["nombre_ubicacion"];
                                    $idubi=$value["id"];
                                }
                                

                                $html .= "<tr>";
                                $html .= "<td>".$codigo_agente."</td>";
                                $html .= "<td>".$nombre_cargo."</td>";
                                $html .= "<td>".$nombre_ubica."</td>";

                                $data_turnos=turnos_ubicacion($idubi);
                                foreach ($data_turnos as $val_turno_ubi) {
                                    $html .= "<td>".(!empty($val_turno_ubi["24hr"]) ? "XX" : "")."</td>";
                                    $html .= "<td>".(!empty($val_turno_ubi["12hde"]) ? "XX" : "")."</td>";
                                    $html .= "<td>".(!empty($val_turno_ubi["12hd6"]) ? "XX" : "")."</td>";
                                    $html .= "<td>".(!empty($val_turno_ubi["12hn6"]) ? "XX" : "")."</td>";
                                    $html .= "<td>".(!empty($val_turno_ubi["12hd7"]) ? "XX" : "")."</td>";
                                    $html .= "<td>".(!empty($val_turno_ubi["12hn7"]) ? "XX" : "")."</td>";
                                    $html .= "<td>".(!empty($val_turno_ubi["septimo"]) ? "XX" : "")."</td>";
                                }
