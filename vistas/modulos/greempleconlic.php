  ...
  <section class="content">
    ...
    <div class="box">
      ...
      <?php
        ...
        function fun_asig_agente() {
            $query = "SELECT * FROM `transacciones_agente` group by idagente_transacciones_agente order by id desc";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        
        function fun_ubicaciones($codigo) {
            $query = "SELECT * FROM `tbl_clientes_ubicaciones` 
                      where codigo_ubicacion='$codigo'";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
        ...
        function fun_empleado_codigo($codigo_agente) {
            $query = "SELECT * FROM tbl_empleados
            WHERE codigo_empleado= '$codigo_agente' and numero_licencia_tenencia_armas != '' ";/* 2023-12-31 formato fecha */  
            $sql = Conexion::conectar()->prepare($query);
            $sql->execute();			
            return $sql->fetchAll();
        };
      ...
      <input type="hidden" class="titulo_reportes" value="PERSONAL SIN LICENCIA DE ARMA">
      ...
      <div class="table-responsive">
                <table  class="table table-bordered table-striped dt-responsive" width="100%" id="tabladatos">
                    <thead>
                        ...
                        <tr>
                            <th class="colspan">LICENCIA DE ARMA</th>
                        </tr>
                        <tr>
                            <th class="colspan">CON LICENCIA</th>
                        </tr>
                    ...
                    <tbody> 
                        <?php
                            ...
                            $html .= "<td>LICENCIA DE PORTACION"."</td>";
                            $html .= "<td>VENCIMIENTO"."</td>";
                            $html .= "</tr>";
                            $numero=1;
                            $data_asig=fun_asig_agente();
                            $array_datos=[];
                            foreach ($data_asig as $val_asig) {
                                $partes = explode("-", $val_asig["nueva_ubicacion_transacciones_agente"]);
                                $codigo=$partes[0];

                                $codigo_agente=$val_asig["idagente_transacciones_agente"];
                                $data_empleado=fun_empleado_codigo($codigo_agente);
                                $cuenta_empleado=0;
                                $nombre_cargo="";
                                $licensia="";
                                $fecha_vencimiento_lpa="";
                                foreach ($data_empleado as $val_empleado) {
                                    ...
                                    $licensia=$val_empleado["numero_licencia_tenencia_armas"];
                                    $fecha_vencimiento_lpa=$val_empleado["fecha_vencimiento_lpa"];
                                    
                                }/* empleado */


                                if($cuenta_empleado>0){
                                    $data_ubicacion=fun_ubicaciones($codigo);
                                        ...
                                        foreach ($data_coordinador as $val_coor) {
                                            ...
                                            foreach ($data_jefe as $val_jefe) {
                                                ...
                                                $array_datos[]=array(
                                                    ...
                                                    "codigo_empleado"=>$codigo_agente,
                                                    "nombre_empleado"=>$nombre_cargo,
                                                    "licencia"=>$licensia,
                                                    "fecha_ven"=>$fecha_vencimiento_lpa,
                                                );
                            ...
                            foreach ($array_datos as $ubicacion) {
                                ...
                                $agrupado[$jefe][$nombre_coor][$name_ubicacion]['detalles'][] = $ubicacion;                  
                            }
                            ...
                            foreach ($agrupado as $jefe => $jefeData) {
                                ...
                                foreach ($jefeData as $nombre_coor => $coorData) {
                                    ...
                                    foreach ($coorData as $nombre_ubi => $ubiData) {
                                        $html.="<tr>";
                                        $html.="<td class='colspan'>".$nombre_ubi."</td>";
                                        $html.="</tr>";
                                        $cuenta_agente=0;
                                        foreach ($ubiData["detalles"] as $val_detalle) {

                                            $fecha_desde_for = strtotime($val_detalle["fecha_ven"]);
                                            $fecha_desde_for = date("Y-m-d", $fecha_desde_for);

                                            $cuenta_agente++;
                                            $html .= "<tr>";
                                            $html .= "<td>".$val_detalle["codigo_empleado"]." ".$val_detalle["nombre_empleado"]."</td>";
                                            $html .= "<td>".$val_detalle["licencia"]."</td>";
                                            $html .= "<td>".$fecha_desde_for."</td>";
                                            $html .= "</tr>";

                                        }/* detalle */
                                        $html.="<tr>";
                                        $html.="<td class='colspan'>SON: ".$cuenta_agente." AGENTES</td>";
                                        $html.="</tr>";
