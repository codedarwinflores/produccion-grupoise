...
class ModeloHorario
{
    ...
    {
        $estado = "EN PROCESO";
        if (strtoupper($datos["estado_exam"]) === "FINALIZADO") {
            $estado = $datos["estado_exam"];
        }
        // Ya existe un registro con la misma fecha inicial y final, entonces actualizamos ese registro
        ...
        if ($stmt->execute([$datos["precio_programar"], $datos["porcentaje_evaluado"], $datos["porcentaje_cliente"], $datos["forma_pago"], $datos["hora_solicitante"], $datos["fecha_solicitante"], $datos["cargo"], $datos["cargo_evaluado_id"], $datos["hora"], $estado, $id])) {
