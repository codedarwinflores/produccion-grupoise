...
class Modeloisr{
	...
	static public function mdlMostrar($tabla, $item, $valor){
		...
		if($item != null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT isr.id as id, `codigo`, `nombre_rango`, `periodo_pago`, `salario_desde`, `salario_hasta`, `base_1`, `tasa_sobre_excedente`, `base_2`,periodo_de_pago.id as idpago, `nombre_periodo`  FROM isr,periodo_de_pago WHERE isr.id = :$item and isr.periodo_pago=periodo_de_pago.id");
			...
			$stmt = Conexion::conectar()->prepare("SELECT isr.id as id, `codigo`, `nombre_rango`, `periodo_pago`, `salario_desde`, `salario_hasta`, `base_1`, `tasa_sobre_excedente`, `base_2`,periodo_de_pago.id as idpago, `nombre_periodo`  FROM isr,periodo_de_pago WHERE isr.periodo_pago=periodo_de_pago.id");
